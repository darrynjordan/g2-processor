...
# $Id: Gnuplot.py,v 1.1 2000/03/22 06:48:10 jasper Exp $
...
class PlotItem:
    ...
    def __init__(self, basecommand, **keyw):
        ...
        if keyw.has_key('extras'):
            self.extras = keyw['extras']
            del keyw['extras']
            self.options.append('extras ' + self.extras)
        if keyw:
...
class Gnuplot:
    ...
    def hardcopy(self, filename=None, eps=0, color=0, enhanced=1,fontsize=0):
        ...
        if fontsize != 0 : setterm.append(`fontsize`)
