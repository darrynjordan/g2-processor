...
RngCmd = 'rngcom.tmp' #IDRoot+'_rngcmd'  # sc001 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
if G2.cfg['$EnableRngProc'] == 'y':
    ...
    if os.system('sudo ./rngcom '+RngCmd) != 0: # run rngcom program  # sc001 added sudo ./
...
if G2.cfg['$EnableStepFreqProc'] != 'off':
    # reset output params for other modules
    ...
    if G2.cfg['$StepFreqLogVer'] != '0.2':
        print 'WARNING - incorrect step freq log version - expecting 0.2 !!!'
    ...
    print '\RncFile = '+RncFile # sc001
       
#------------
# Corner Turn
if G2.cfg['$EnableCornerTurn'] == 'y':
    AzSamplesLessOne = int(G2.wrk['$AzSamples'])-1
    print 'AzSamplesLessOne = ' + repr(AzSamplesLessOne)  # sc001
    cmd = 'sudo ./corner '+cornerInFile+' '+CorFile+' '+G2.wrk['$RngSamples']+' 0 '+\
          `AzSamplesLessOne`+' 8 mem=50' # sc001 added sudo ./
    print 'cmd = ' + cmd  # sc001
