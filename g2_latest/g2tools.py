...
class G2Config:
    ...
    def parseParam(self,FileName,SearchStr,KeyStr,dict):
        ...
        while 1:    
            ...
            if not str1:
                break
            if string.find(str1,SearchStr) != -1: # find line with SearchStr  
                field = string.split(str1)
    ...
    def versionControl(self):
        if string.atof(self.cfg['$RadarConfigVersion']) < 0.4:
        ...
        if string.atof(self.cfg['$ProcConfigVersion']) < 0.6:
            self.cfg['$Float2ByteMath'] = 'none'
            print 'Version control - setting Float2ByteMath to none'

        if string.atof(self.cfg['$ProcConfigVersion']) < 0.7:
...
#---------------------------        
    def createRngProcCmdFile(self,FileName):
        ...
        '$RngComFlg [Y/N - SRC]         => '\
                     +string.upper(self.cfg['$RngCompress'])+'\n'+\
        ...
        '$MoCompFlg [Y/N]                => '\
                      +string.upper(self.cfg['$MoComp'])+'\n'+\
        '$MoCompFileName                 => '+self.cfg['$MocFile']+'\n'+\
        '$MoCompRngShiftFlg [Y/N]        => '\
                      +string.upper(self.cfg['$MoCompRngShiftFlg'])+'\n'+\
...
#--------------------------------
    def createStepFreqProcCmdFile(self,FileName):
        ...
        '$HsysFileName                     => '+self.cfg['$HsysFileName']+'\n'+\
        '$HsysRngBins                      => '+self.cfg['$HsysRngBins']+'\n'+\
        '$HsysSFSteps                      => '+self.cfg['$HsysSFSteps']+'\n'+\
        '$HsysStartFrequ                   => '+self.cfg['$HsysStartFrequ']+'\n'+\
        '$HsysStepSize                     => '+self.cfg['$HsysStepSize']+'\n'+\
        '$CalAvgBursts                     => '+self.cfg['$CalAvgBursts']+'\n'+\
        '$InputDataType [note]             => 3\n'+\
        '$StepfDataPhaseSign               => '+self.cfg['$StepfDataPhaseSign']+'\n'+\
        ...
        '$RngComRefFuncPhaseSign [+-1]     => '+self.cfg['$RngComRefPhase']+'\n'+\
        '$StepFreqCombineMode (splice/add) => '+self.cfg['$StepFreqCombineMode']+'\n'+\
        '$DFTSpeed (estimate/measure)      => '+self.cfg['$DFTSpeed']+'\n'+\
